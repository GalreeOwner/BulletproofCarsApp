...
export default function AdminIngestPage() {
  ...
  async function ingestDealer() {
    ...
    const res = await fetch("/api/ingest/dealer", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: dealerUrl, name: dealerName }),
    });
    const json = await res.json();
    setResult(JSON.stringify(json, null, 2));
  }
    ...
    <main className="p-6 max-w-3xl mx-auto">
      ...
      <div className="mt-6 border rounded-xl p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3 mt-3">
          <input className="border rounded-lg p-2" value={make} onChange={(e) => setMake(e.target.value)} />
          <input className="border rounded-lg p-2" value={model} onChange={(e) => setModel(e.target.value)} />
          <input className="border rounded-lg p-2" value={year} onChange={(e) => setYear(e.target.value)} />
        </div>
        <button className="mt-4 rounded-lg bg-black text-white px-4 py-2" onClick={ingestNhtsa}>
      ...
      <div className="mt-6 border rounded-xl p-4">
        ...
        />
        <button className="mt-4 rounded-lg bg-black text-white px-4 py-2" onClick={ingestDealer}>
          Ingest dealer page
        </button>
      </div>
